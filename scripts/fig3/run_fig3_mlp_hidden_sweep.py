#!/usr/bin/env python3
"""
MLP hidden_dim sweep under strict Fig.3 MNN pseudo-label protocol.

MNN pseudo labels are built once (shared across hidden_dim configs).
Default grid: hidden_dim in {32, 64, 128, 256, 512}
Default panel: data/panel_split_official.csv
"""

import os
import sys

PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
FIG3_DIR = os.path.join(PROJECT_ROOT, 'scripts', 'fig3')
sys.path.insert(0, PROJECT_ROOT)
sys.path.insert(0, FIG3_DIR)
sys.path.insert(0, os.path.join(PROJECT_ROOT, 'experiments', 'fig3'))
from exp_paths import output_dir

import argparse
import contextlib
import gc
import io
import numpy as np
import pandas as pd
import torch
from tqdm import tqdm

import SpatialEx as se
import run_fig3_mnn_pseudo as mnn


DEFAULT_HIDDEN_DIMS = [32, 64, 128, 256, 512]


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--data_root', type=str, default=os.path.join(PROJECT_ROOT, 'data'))
    parser.add_argument('--rep1', type=str, default='Human_Breast_Cancer_Rep1_uni_resolution64_full.h5ad')
    parser.add_argument('--rep2', type=str, default='Human_Breast_Cancer_Rep2_uni_resolution64_full.h5ad')
    parser.add_argument('--panel_csv', type=str,
                        default=os.path.join(PROJECT_ROOT, 'data', 'panel_split_official.csv'))
    parser.add_argument('--panelA_size', type=int, default=150)
    parser.add_argument('--hidden_dims', type=int, nargs='+', default=DEFAULT_HIDDEN_DIMS)
    parser.add_argument('--k', type=int, default=5)
    parser.add_argument('--mnn_k', type=int, default=20)
    parser.add_argument('--epochs', type=int, default=300)
    parser.add_argument('--lr', type=float, default=1e-3)
    parser.add_argument('--device', type=str, default='cuda:0')
    parser.add_argument('--batch_size', type=int, default=1024,
                        help='kNN mini-batch size for MNN pseudo-label construction')
    parser.add_argument('--seed', type=int, default=0)
    parser.add_argument('--resume', action='store_true')
    parser.add_argument('--quiet', action='store_true')
    parser.add_argument('--out_dir', type=str, default=output_dir('mlp_hidden_sweep_official'))
    return parser.parse_args()


def load_panels(args, rep1):
    if args.panel_csv and os.path.exists(args.panel_csv):
        panel_df = pd.read_csv(args.panel_csv)
        panelA = panel_df[panel_df['panel'] == 'panelA']['gene'].astype(str).tolist()
        panelB = panel_df[panel_df['panel'] == 'panelB']['gene'].astype(str).tolist()
        print(f'Loaded panel split from {args.panel_csv}: A={len(panelA)}, B={len(panelB)}', flush=True)
        return panelA, panelB

    genes = rep1.var_names.values
    np.random.seed(args.seed)
    np.random.shuffle(genes)
    panelA = genes[:args.panelA_size].tolist()
    panelB = genes[args.panelA_size:].tolist()
    print(f'Random panel split: A={len(panelA)}, B={len(panelB)}', flush=True)
    return panelA, panelB


def free_device_memory(device):
    gc.collect()
    if device.type == 'cuda':
        torch.cuda.empty_cache()


def build_mnn_pseudo_quiet(*args, quiet=False, **kwargs):
    ctx = contextlib.redirect_stdout(io.StringIO()) if quiet else contextlib.nullcontext()
    with ctx:
        return mnn.build_mnn_pseudo(*args, **kwargs)


def predict(model, x, y_mean, y_std, device):
    model.eval()
    with torch.no_grad():
        pred = model(torch.tensor(mnn.zscore(x), dtype=torch.float32, device=device)).cpu().numpy()
    return pred * y_std + y_mean


def train_panel_mlp_quiet(X1, Y1_pseudo, X2, Y2_pseudo, in_dim, out_dim1, out_dim2,
                          hidden_dim, epochs, lr, device, quiet=False):
    import torch.nn as nn

    model1 = mnn.PanelMLP(in_dim, hidden_dim, out_dim1).to(device)
    model2 = mnn.PanelMLP(out_dim1, hidden_dim, out_dim2).to(device)
    opt = torch.optim.Adam(list(model1.parameters()) + list(model2.parameters()), lr=lr)
    crit = nn.MSELoss()
    X1_t = torch.tensor(mnn.zscore(X1), dtype=torch.float32, device=device)
    X2_t = torch.tensor(mnn.zscore(X2), dtype=torch.float32, device=device)
    Y1_t = torch.tensor(mnn.zscore(Y1_pseudo), dtype=torch.float32, device=device)
    Y2_t = torch.tensor(mnn.zscore(Y2_pseudo), dtype=torch.float32, device=device)
    epoch_iter = tqdm(range(epochs), desc=f'train h={hidden_dim}', leave=not quiet, disable=quiet)
    for _ in epoch_iter:
        model1.train()
        model2.train()
        opt.zero_grad()
        loss = crit(model1(X1_t), Y1_t) + crit(model2(X2_t), Y2_t)
        loss.backward()
        opt.step()
    return model1, model2


def load_done_dims(out_dir):
    out_csv = os.path.join(out_dir, 'mlp_hidden_sweep.csv')
    if not os.path.exists(out_csv):
        return set()
    return set(int(x) for x in pd.read_csv(out_csv)['hidden_dim'].tolist())


def append_row(row, out_dir):
    out_csv = os.path.join(out_dir, 'mlp_hidden_sweep.csv')
    chunk = pd.DataFrame([row])
    if os.path.exists(out_csv):
        pd.concat([pd.read_csv(out_csv), chunk], ignore_index=True).to_csv(out_csv, index=False)
    else:
        chunk.to_csv(out_csv, index=False)
    print(f'Appended hidden_dim={row["hidden_dim"]} -> {out_csv}', flush=True)


def run_config(hidden_dim, Y_A1, pseudo_B1, Y_B1, Y_B2, pseudo_A2, Y_A2,
               graph1, graph2, args, device):
    print(f'\n=== hidden_dim={hidden_dim} ===', flush=True)
    model1, model2 = train_panel_mlp_quiet(
        Y_A1, pseudo_B1, Y_B2, pseudo_A2,
        Y_A1.shape[1], Y_B2.shape[1], Y_A1.shape[1],
        hidden_dim, args.epochs, args.lr, device, quiet=args.quiet)

    yB_mean, yB_std = Y_B2.mean(axis=0), Y_B2.std(axis=0) + 1e-8
    yA_mean, yA_std = Y_A1.mean(axis=0), Y_A1.std(axis=0) + 1e-8
    pred_B1 = predict(model1, Y_A1, yB_mean, yB_std, device)
    pred_A2 = predict(model2, Y_B2, yA_mean, yA_std, device)
    learned1 = mnn.evaluate(Y_B1, pred_B1, graph1)
    learned2 = mnn.evaluate(Y_A2, pred_A2, graph2)

    direct1 = mnn.evaluate(Y_B1, pseudo_B1, graph1)
    direct2 = mnn.evaluate(Y_A2, pseudo_A2, graph2)

    n_params = sum(p.numel() for p in list(model1.parameters()) + list(model2.parameters()))
    del model1, model2, pred_B1, pred_A2
    free_device_memory(device)

    return {
        'hidden_dim': hidden_dim,
        'k': args.k,
        'mnn_k': args.mnn_k,
        'epochs': args.epochs,
        'lr': args.lr,
        'n_params': n_params,
        'slice1_direct_pcc': direct1[0],
        'slice1_direct_ssim': direct1[1],
        'slice1_learned_pcc': learned1[0],
        'slice1_learned_ssim': learned1[1],
        'slice2_direct_pcc': direct2[0],
        'slice2_direct_ssim': direct2[1],
        'slice2_learned_pcc': learned2[0],
        'slice2_learned_ssim': learned2[1],
    }


def main():
    args = parse_args()
    os.makedirs(args.out_dir, exist_ok=True)
    device = torch.device(args.device if torch.cuda.is_available() else 'cpu')
    np.random.seed(args.seed)
    torch.manual_seed(args.seed)

    rep1 = mnn.load_slice(args.rep1, args.data_root)
    rep2 = mnn.load_slice(args.rep2, args.data_root)
    panelA, panelB = load_panels(args, rep1)

    Y_A1 = mnn.get_X(rep1, panelA).astype(np.float32)
    Y_B1 = mnn.get_X(rep1, panelB).astype(np.float32)
    Y_A2 = mnn.get_X(rep2, panelA).astype(np.float32)
    Y_B2 = mnn.get_X(rep2, panelB).astype(np.float32)
    HE1 = np.asarray(rep1.obsm['he'], dtype=np.float32)
    HE2 = np.asarray(rep2.obsm['he'], dtype=np.float32)

    graph1 = se.pp.Build_graph(rep1.obsm['spatial'], graph_type='knn', weighted='gaussian',
                               apply_normalize='row', return_type='coo')
    graph2 = se.pp.Build_graph(rep2.obsm['spatial'], graph_type='knn', weighted='gaussian',
                               apply_normalize='row', return_type='coo')

    print(f'Building strict MNN pseudo labels (k={args.k}, mnn_k={args.mnn_k})...', flush=True)
    bs = args.batch_size
    pseudo_B1 = build_mnn_pseudo_quiet(
        HE1, HE2, Y_B2, k=args.k, mnn_k=args.mnn_k, batch_size=bs, device=device, quiet=args.quiet)
    free_device_memory(device)
    pseudo_A2 = build_mnn_pseudo_quiet(
        Y_B2, pseudo_B1, Y_A1, k=args.k, mnn_k=args.mnn_k, batch_size=bs, device=device, quiet=args.quiet)
    free_device_memory(device)

    done = load_done_dims(args.out_dir) if args.resume else set()
    pending = [h for h in args.hidden_dims if h not in done]
    print(f'hidden_dim sweep: {len(args.hidden_dims)} total, {len(done)} done, {len(pending)} pending',
          flush=True)
    if pending:
        print('Pending:', pending, flush=True)

    for hidden_dim in pending:
        row = run_config(
            hidden_dim, Y_A1, pseudo_B1, Y_B1, Y_B2, pseudo_A2, Y_A2,
            graph1, graph2, args, device)
        append_row(row, args.out_dir)

    out_csv = os.path.join(args.out_dir, 'mlp_hidden_sweep.csv')
    if not os.path.exists(out_csv):
        print('No results produced.', flush=True)
        return

    df = pd.read_csv(out_csv).sort_values('hidden_dim')
    print('\n=== MLP hidden_dim sweep (official split, strict MNN) ===', flush=True)
    cols = ['hidden_dim', 'n_params', 'slice1_learned_pcc', 'slice1_learned_ssim',
            'slice2_learned_pcc', 'slice2_learned_ssim']
    print(df[cols].to_string(index=False), flush=True)

    best_s1 = df.loc[df['slice1_learned_pcc'].idxmax()]
    best_s2 = df.loc[df['slice2_learned_pcc'].idxmax()]
    print(f'\nBest Slice1 learned PCC: hidden_dim={int(best_s1.hidden_dim)}, '
          f'PCC={best_s1.slice1_learned_pcc:.4f}', flush=True)
    print(f'Best Slice2 learned PCC: hidden_dim={int(best_s2.hidden_dim)}, '
          f'PCC={best_s2.slice2_learned_pcc:.4f}', flush=True)


if __name__ == '__main__':
    main()
