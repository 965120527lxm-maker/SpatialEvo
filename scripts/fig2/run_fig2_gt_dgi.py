#!/usr/bin/env python3
"""
Fig.2: Graph Transformer + DGI (reconstruction + contrastive).

Same protocol as run_fig2_gt.py, but replaces MFP with DGI on GT encoder
embeddings (shuffle H&E features, re-encode, contrast against graph summary).
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Fig.2 GT+DGI H&E-to-omics")
    ...
    p.add_argument("--dgi_weight", type=float, default=1.0)
    p.add_argument("--epochs", type=int, default=500)
    p.add_argument("--lr", type=float, default=1e-3)
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--device", type=str, default="cuda:0")
    p.add_argument("--out_dir", type=str, default=output_dir("gt_dgi_breast_cancer"))
    return p.parse_args()
...
def main():
    ...
    print(
        f"Training SpatialExGT+DGI: {adata1.n_obs} + {adata2.n_obs} cells, "
        f"{len(genes)} genes, hidden_dim={args.hidden_dim}, dgi_weight={args.dgi_weight}"
    )
    model = SpatialExGT(
    ...
    summary.to_csv(os.path.join(args.out_dir, "metrics_gt_dgi.csv"), index=False)
