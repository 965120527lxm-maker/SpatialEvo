        ...
        if self.platform == 'Xenium':
            for epoch in tqdm(range(self.epochs)):
                ...
                for data1, data2 in batch_iter:
                    ...
                    loss3, _ = self.rm_AB(panel_1a, panel_1b, torch.spmm(agg_mtx1, panel_1b[selection1]), agg_mtx1, self.use_agg) ##将切片1的组学a映射成切片1的组学b，与预测的组学b进行比较
                    loss4, _ = self.rm_BA(panel_2b, panel_2a, torch.spmm(agg_mtx2, panel_2a[selection2]), agg_mtx2, self.use_agg) ##将切片2的组学b映射成切片2的组学a，与预测的组学a进行比较
